...
const v = {
  room: {
    offeredItems: new DefaultMap<PersonIndex, Set<PickupIndex>>(() => new Set()),
  },
};
...
export function isPersonOfferingItem(person: PersonIndex, item: EntityPickupCollectible): boolean {
  return v.room.offeredItems.getAndSetDefault(person).has(mod.getPickupIndex(item));
}
...
function postUpdate() {
  ...
  getAllPersons().forEach((person) => {
    ...
    if (
      ...
      ].every((action) => Input.IsActionPressed(action, player.ControllerIndex)) ||
...
function offerAvailableItems(player: EntityPlayer) {
  ...
  getCollectibles()
    ...
    .forEach((collectible) => {
      ...
      if (owner === person) {
        v.room.offeredItems.getAndSetDefault(person).add(mod.getPickupIndex(collectible));
      }
    });
}
